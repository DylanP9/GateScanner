...
export default function AirportReportCard({ report }: AirportReportCardProps) {
  const combinedMinutes = report.securityMinutes + report.passportMinutes;
    ...
    <article className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
      ...
      <div className="mt-6 space-y-3">
        <div className="flex items-center justify-between rounded-2xl bg-slate-50 p-3">
          ...
          <span className="text-xl font-black">{report.securityMinutes}m</span>
        </div>

        <div className="flex items-center justify-between rounded-2xl bg-slate-50 p-3">
          <span className="text-sm font-bold text-slate-600">Passport</span>
          <span className="text-xl font-black">{report.passportMinutes}m</span>
        </div>
